...
int executarMetodo(method_info *metodo, ClassFile *classe, Pilha_frames *pilha) {
    ...
    while(!execucao) {
        ...
        execucao = executarInstrucoes(metodo, metodoAtual); // TO DO: Função que executa intruções
...
int executarInstrucoes (method_info *metodo, Frame *frame){ // TO DO: Implementar execução das instruções
  	return 45;
}
